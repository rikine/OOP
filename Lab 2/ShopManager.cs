...
namespace Shops
{
    class ShopManager
    {
        ...
        {
            var shopIn = shops.Find(x => x.id == shop.id);
            if (shopIn == null) throw new AddProductException($"Can't add a {product}. {shop.name} {shop.id} doesn't exists.");
            var prodIn = products.Find(x => x.id == product.id);
            if (prodIn == null) throw new AddProductException($"Can't add a {product} because it doesn't exist.");

            if (qty < 1) throw new AddProductException($"Can't add a product with < 1 quantity. Your quantity is {qty}");
            if (price == -100.0)
                shopIn.Add(prodIn, (uint)qty);
            else
            {
                if (price < 0)
                    throw new AddProductException($"Price can't be below 0.");
                shopIn.Add(prodIn, (uint)qty, price);
            }
        }
        ...
        public bool TryBuy(Shop shop, Product product, int qty, out double totalPrice)
        {
            totalPrice = 0;
            var shopIn = shops.Find(x => x.id == shop.id);
            if (shopIn == null) return false;
            var prodIn = products.Find(x => x.id == product.id);
            if (prodIn == null) return false;

            if (!shopIn.TryBuy(prodIn, (uint)qty, out double price))
                return false;
            totalPrice = price;
            return true;
        }
        ...
        public Shop ShopWithCheapestProduct(Product product)
        {
            ...
            {
                if (shop.GetShipments().Find(x => x.id == product.id) != null
        ...
        {
            var shopIn = shops.Find(x => x.id == shop.id);
            if (shopIn == null) throw new AddProductException($"Can't find max buys in {shop.name} {shop.id} because it doesn't exists.");

            var buys = new List<(Product product, int qty)>();
            foreach (var prod in shopIn.GetShipments())
            {
        ...
        public Shop CheapestShopForShipment(List<(Product product, int qty)> listOfProducts)
        {
            ...
            foreach (var shop in shops)
            {
                ...
                {
                    if (shop.GetShipments().Find(x => x.id == prod.product.id) == null)
                    {
                        ready = false;
                        break;
                    }
